...
export default function DrawerLayout() {
  ...
  useEffect(() => {
    if (navigationState?.routes) {
      const drawerRoute = navigationState.routes.find((route) => route.name === "(drawer)");
      // Aqui você pode adicionar lógica para detectar se o drawer está aberto
    }
  }, [navigationState]);
      ...
      <>
        ...
        <DrawerContentScrollView {...props}>
          <View style={styles.drawerContainer}>
            ...
            {/* 🔥 Botão de navegação para Configurações */}
            <TouchableOpacity style={styles.menuItem} onPress={() => router.push('/setting')}>
              <Ionicons name="settings-outline" size={24} color="white" />
              <Text style={styles.menuText}>Configurações</Text>
            </TouchableOpacity>
  ...
  return (
    <>
      ...
      >
        {/* 🔥 Stack para permitir header nas páginas acessadas pelo drawer (ex: /setting) */}
        <Stack>
        ...
        <Stack.Screen
          ...
          options={{
            title: 'Configurações',
            headerStyle: {
              backgroundColor: '#0475FF', // Fundo azul
            },
            headerTintColor: '#fff', // Ícones e título brancos
